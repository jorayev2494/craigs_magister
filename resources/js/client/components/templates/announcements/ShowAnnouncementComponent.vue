<template>
	<div>
		<show-head-announcement-component :announcement="announcement" v-if="announcement"></show-head-announcement-component>
			...
			<div class="container">
				<div class="row">
						...
						<div class="row">
							...
							<div class="col-lg-12">
								<div class="listing_single_description">
									<div class="lsd_list">
										<ul class="mb0">
											<!-- <li class="list-inline-item">
												<a href="#">
													Location: {{ announcement.location.city }}, {{ announcement.location.country }}
												</a>
											</li> -->
											<li class="list-inline-item">
												<a href="#">
											...
											<!-- <li class="list-inline-item">
												<a href="#">
													Reviews: {{ announcement.reviews.length }}
												</a>
											</li> -->
									...
									<h4 class="mb30">Description</h4>
									<p class="mb25">{{ announcement.description }}</p>
									<!-- <p class="gpara second_para white_goverlay mt10 mb10">Fully furnished. Elegantly appointed condominium unit situated on premier location. PS6. The wide entry hall leads to a large living room with dining area. This expansive 2 bedroom and 2 renovated marble bathroom apartment has great windows. Despite the interior views, the apartments Southern and Eastern exposures allow for lovely natural light to fill every room. The master suite is surrounded by handcrafted milkwork and features incredible walk-in closet and storage space.</p>
									<div class="collapse" id="collapseExample">
										<div class="card card-body">
											<p class="mt10 mb10">Fully furnished. Elegantly appointed condominium unit situated on premier location. PS6. The wide entry hall leads to a large living room with dining area. This expansive 2 bedroom and 2 renovated marble bathroom apartment has great windows. Despite the interior views, the apartments Southern and Eastern exposures allow for lovely natural light to fill every room. The master suite is surrounded by handcrafted milkwork and features incredible walk-in closet and storage space.</p>
									...
									<!-- <p class="overlay_close" v-if="announcement.description.length > 250">
							...
							<div class="col-lg-12">
								<div class="additional_details">
									<div class="row">
										...
										<div class="col-md-6 col-lg-6 col-xl-4" v-for="(val, key, index) in announcement.concrete" :key="index">
											<!-- {{ val }} | {{ key }} | {{ index }} -->
											<ul class="list-inline-item">
												<li>
													<p>{{ $t(`concrete_announcement.${key}`) }} :</p>
												</li>
												<!-- <li>
													<p>Price :</p>
												</li>
							...
							<block-reviews-component 
								:reviews="announcement.reviews" 
								:rate="announcement.rate" 
								:type="'announcement'"
								:settings="{
											width: announcement.reviews.length ? '100%' : '243%',
											showFormBlock: authUser.id != announcement.creator.id
										}">
							</block-reviews-component>
					...
					<div class="col-lg-4 col-xl-4">
						<div class="sidebar_listing_list">
							<div class="sidebar_advanced_search_widget">
								<div class="sl_creator">
									...
									<div class="media">
										<img class="mr-3 avatar-radius" v-server-image:90="announcement.creator.avatar" />
										<div class="media-body">
											<h5 class="mt-0 mb0">{{ announcement.creator.full_name }}</h5>
											<p class="mb0">{{ announcement.creator.phone }}</p>
											<p class="mb0">{{ announcement.creator.email }}</p>
											<router-link class="text-thm" :to="{ name: 'user-profile', params: { user_id: announcement.creator.id } }">
												View My Listing
											</router-link>
								...
								<ul class="sasw_list mb0">
									<li class="search_area">
										<div class="form-group">
											<input type="text" class="form-control" id="exampleInputName1" v-model="send_message_creator.your_name" required :disabled="authUser.id == announcement.creator.id" placeholder="Your Name">
										</div>
									</li>
									<li class="search_area">
										<div class="form-group">
											<input type="number" class="form-control" id="exampleInputName2" v-model="send_message_creator.your_phone" required :disabled="authUser.id == announcement.creator.id" placeholder="Phone">
										</div>
									</li>
									<li class="search_area">
										<div class="form-group">
											<input type="email" class="form-control" id="exampleInputEmail" v-model="send_message_creator.your_email" required :disabled="authUser.id == announcement.creator.id" placeholder="Email">
										</div>
									</li>
									<li class="search_area">
										<div class="form-group">
											<textarea id="form_message" name="form_message" class="form-control required" rows="5" required="required" v-model="send_message_creator.your_description" :disabled="authUser.id == announcement.creator.id" placeholder="I'm interest in [ Listing Single ]"></textarea>
										</div>
									</li>
									<li>
										<div class="search_option_button">
											<button type="submit" class="btn btn-block btn-thm" @click.prevent="sendMessageCreator($event)" :disabled="authUser.id == announcement.creator.id">Send</button>
										</div>
									</li>
								</ul>
							</div>
						...
						<sidebar-category-component v-if="categories" :categories="categories"></sidebar-category-component>
...
<script>
	...
	export default {
		...
		methods: {
			async sendMessageCreator(event) {
				await this.$httpClient.post(`/api/send_message_creator/${this.$route.params.id}?type=announcement`, { 
						creator_id: this.announcement.creator.id,
						...this.send_message_creator,
					}).then((response) => {
					if (response.status == 202) {
						this.send_message_creator.your_name
						= this.send_message_creator.your_phone
						= this.send_message_creator.your_email
						= this.send_message_creator.your_description = '';

						this.$notify({
							group: 'server-response',
							type: 'success',
							title: 'Message successfull sent!',
							text: 'Message successfull sent:' + this.announcement.creator.full_name
						});				
					}
				}).catch((err) => {
		...
		components: {
            ...
            SidebarCategoryComponent: () => import('../../includes/sidebar/SidebarCategoryComponent.vue'),
		},
		async created() {
			await this.$httpClient.get(`/api/announcements/${this.$route.params.id}`).then((response) => {
				 this.announcement = response.data;
			}).catch((err) => { });
			
			await this.$httpClient.get('/api/server/categories').then((response) => {
